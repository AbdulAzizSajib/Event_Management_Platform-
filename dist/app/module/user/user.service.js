...
const getMyDashboard = async (userId) => {
    ...
    const [
    // Counts
    organizedCount, participationCount, pendingInvitations, reviewCount, savedCount, totalSpent, 
    // Participation status breakdown
    pendingParticipations, approvedParticipations, rejectedParticipations, 
    // Upcoming events (organized by me)
    upcomingOrganized, 
    // Upcoming events (participating in)
    upcomingParticipating, 
    // Recent data
    recentOrganizedEvents, recentParticipations, recentInvitations, recentReviews, recentSavedEvents,] = await Promise.all([
        // Counts
