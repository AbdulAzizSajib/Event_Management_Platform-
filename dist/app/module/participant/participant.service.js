...
const joinEvent = async (userId, eventId) => {
    ...
    if (existingParticipant) {
        ...
        throw new AppError(status.CONFLICT, "You have already joined this event");
    }
...
const getEventParticipants = async (eventId, query) => {
    const { page = "1", limit = "10", sortBy = "joinedAt", sortOrder = "desc", status: participantStatus, } = query;
...
const getMyParticipations = async (userId, query) => {
    const { page = "1", limit = "10", sortBy = "joinedAt", sortOrder = "desc", status: participantStatus, } = query;
    const pageNum = Math.max(Number(page), 1);
    const limitNum = Math.max(Number(limit), 1);
    const skip = (pageNum - 1) * limitNum;
...
const updateParticipantStatus = async (participantId, userId, newStatus) => {
    ...
    if (participant.event.organizerId !== userId) {
        throw new AppError(status.FORBIDDEN, "Only the event organizer can update participant status");
    }
    ...
    if (participant.status === ParticipantStatus.BANNED &&
        ...
        throw new AppError(status.BAD_REQUEST, "Banned participant can only be unbanned (set to APPROVED)");
    }
...
const cancelParticipation = async (eventId, userId) => {
    ...
    if (participant.status === ParticipantStatus.BANNED) {
        throw new AppError(status.FORBIDDEN, "You have been banned from this event and cannot cancel");
    }
