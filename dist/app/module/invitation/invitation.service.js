...
import { InvitationStatus, ParticipantStatus, } from "../../../generated/prisma/enums.js";
import AppError from "../../errorHelpers/AppError.js";
import { prisma } from "../../lib/prisma.js";
import { sendEmail } from "../../utils/email.js";
import { envVars } from "../../config/env.js";
const sendInvitation = async (inviterId, eventId, inviteeId) => {
    ...
    if (event.organizerId !== inviterId) {
        throw new AppError(status.FORBIDDEN, "Only the event organizer can send invitations");
    }
    ...
    if (existingInvitation) {
        throw new AppError(status.CONFLICT, "This user has already been invited to this event");
    }
    ...
    if (existingParticipant) {
        throw new AppError(status.CONFLICT, "This user is already a participant of this event");
    }
...
const respondToInvitation = async (invitationId, userId, responseStatus) => {
    ...
    if (invitation.inviteeId !== userId) {
        throw new AppError(status.FORBIDDEN, "Only the invited user can respond to this invitation");
    }
...
const getMyInvitations = async (userId, query) => {
    const { page = "1", limit = "10", sortBy = "createdAt", sortOrder = "desc", status: invitationStatus, } = query;
...
const getSentInvitations = async (userId, eventId, query) => {
    const event = await prisma.event.findUnique({
        where: { id: eventId },
    });
    if (!event) {
    ...
    if (event.organizerId !== userId) {
        throw new AppError(status.FORBIDDEN, "Only the event organizer can view sent invitations");
    }
    const { page = "1", limit = "10", sortBy = "createdAt", sortOrder = "desc", status: invitationStatus, } = query;
    const pageNum = Math.max(Number(page), 1);
    const limitNum = Math.max(Number(limit), 1);
...
const cancelInvitation = async (invitationId, userId) => {
    ...
    if (invitation.inviterId !== userId) {
        throw new AppError(status.FORBIDDEN, "Only the event organizer can cancel invitations");
    }
    if (invitation.status !== InvitationStatus.PENDING) {
        throw new AppError(status.BAD_REQUEST, `Cannot cancel an invitation that has been ${invitation.status.toLowerCase()}`);
    }
