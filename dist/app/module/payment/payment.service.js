...
import { PaymentMethod, PaymentStatus, ParticipantStatus, } from "../../../generated/prisma/enums.js";
import AppError from "../../errorHelpers/AppError.js";
import { prisma } from "../../lib/prisma.js";
import { stripe } from "../../lib/stripe.js";
import { envVars } from "../../config/env.js";
import { sendEmail } from "../../utils/email.js";
const createCheckoutSession = async (userId, eventId) => {
    ...
    if (Number(event.fee) === 0) {
        throw new AppError(status.BAD_REQUEST, "This is a free event. No payment required.");
    }
    if (event.organizerId === userId) {
        throw new AppError(status.BAD_REQUEST, "You cannot pay for your own event");
    }
    ...
    if (existingPayment) {
        throw new AppError(status.CONFLICT, "You have already paid for this event");
    }
    // Check if banned
...
const handleStripeWebhookEvent = async (event) => {
    ...
    switch (event.type) {
        case "checkout.session.completed": {
            ...
            if (!eventData) {
                console.error(`⚠️ Event ${eventId} not found. Payment may be for deleted event.`);
            ...
            console.log(`✅ Payment ${session.payment_status} for event ${eventId}`);
            break;
        }
        case "checkout.session.expired": {
            ...
            console.log(`Checkout session ${session.id} expired. Payment marked as cancelled.`);
            break;
        }
...
const getPaymentsByEvent = async (eventId, userId, query) => {
    const event = await prisma.event.findUnique({
        where: { id: eventId },
    });
    if (!event) {
    ...
    if (event.organizerId !== userId) {
        throw new AppError(status.FORBIDDEN, "Only the event organizer can view payments");
    }
