        ...
        if (sessionToken) {
            ...
            if (sessionExits && sessionExits.user) {
                ...
                if (percentRemaining < 20) {
                    ...
                    console.log(`Session token is nearing expiration. Time remaining: ${percentRemaining.toFixed(2)}%`);
                }
                if (user.status === UserStatus.BLOCKED ||
                    user.status === UserStatus.DELETED) {
                    throw new AppError(status.UNAUTHORIZED, "User is not allowed to access this resource.");
                }
                if (user.isDeleted) {
                    throw new AppError(status.UNAUTHORIZED, "User account has been deleted.");
                }
